...
@click.option('--template_file', default='es_mappings/pfl_template.json',
...
def es_put_template(template_file):
    ...
    that should be applied to multiple indices. Check ``es_mappings/pfl_template.json``
    for an example.

    :param template_file: Path to JSON file containing the template. Defaults to ``es_mappings/pfl_template.json``.
    """
    ...
    es.indices.put_template('pfl_template', template)
...
def create_indexes(mapping_dir):
    ...
    following naming convention: "pfl_mapping_{SOURCE_NAME}.json".
    """
    click.echo('Creating indexes for ES mappings in %s' % (mapping_dir))

    for mapping_file_path in glob('%s/pfl_mapping_*.json' % mapping_dir):
...
def delete_indexes(delete_template):
    ...
    if delete_template or click.confirm('Do you want to delete the template too?'):
        es.indices.delete_template('pfl_template')
...
def load_dump(collection_dump, collection_name):
    ...
    :param collection_name: Name for the local index to restore the dump to. Optional; will be derived from the dump name, at your own risk. Note that the pipeline will add a "pfl_" prefix string to the collection name, to ensure the proper mapping and settings are applied.
    """
    ...
    if not collection_name:
        collection_name = collection_id.replace('pfl_', '')
